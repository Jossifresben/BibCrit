...
**Corpus layer.** The `BiblicalCorpus` class loads CSV exports of six corpus traditions at startup: ETCBC (MT, all 39 books), Rahlfs LXX (38 books), ETCBC DSS (1QIsaᵃ, 4QSamᵃ, 11QPaleoLev, 4QDeutn), Samaritan Pentateuch (Torah, 5 books), ETCBC Peshitta (Syriac OT, all 39 books, 308,863 word tokens), and SBLGNT (NT, 27 books). It resolves verse references across traditions and returns typed word objects carrying lemma, morphology, and Strong's identifiers. This layer is entirely deterministic and requires no API calls.
...
The eleven analytical tools, their scholarly methods, and active prompt versions are summarised in Table 1.
