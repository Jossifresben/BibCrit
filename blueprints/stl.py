"""Second Temple Literature Bridge — /stl page + /api/stl/stream SSE endpoint."""
...
from biblical_core.claude_pipeline import STL_MODEL, CACHE_META_KEYS
from biblical_core.ref_utils import estimate_verse_count, TOOL_VERSE_LIMITS
import state


stl_bp = Blueprint('stl', __name__)

# IMPORTANT: must stay in sync with prompt_version in pipeline.analyze_stl()
_STL_PROMPT = 'v1'
...
_STEPS = {
    'en': {
        ...
        'generating':     'Mapping Second Temple Literature connections — this typically takes 60–90 seconds…',
    ...
    'es': {
        ...
        'generating':     'Mapeando conexiones con la Literatura del Segundo Templo — esto tarda 60–90 segundos…',
...
def _check_ref_length(reference: str) -> str | None:
    max_v = TOOL_VERSE_LIMITS.get('stl')
    if not max_v:
        return None
...
def stl():
    ...
    return render_template('stl.html', lang=lang, reference=reference, t=state.t)


# ── SSE stream ──────────────────────────────────────────────────────────────

@stl_bp.route('/api/stl/stream')
def api_stl_stream():
    ...
    def generate():
        ...
        # Load MT and LXX texts for context (gracefully empty if not in corpus)
        ...
        # Emit corpus texts so the client can populate text columns immediately
        ...
        if lang == 'es':
            cached_es = pipeline.get_cached_es(reference, 'stl', _STL_PROMPT, STL_MODEL)
            if cached_es:
        ...
        cached = pipeline.get_cached(reference, 'stl', _STL_PROMPT, STL_MODEL)
        ...
        if cached:
            ...
            def _run_stl():
                try:
                    for key, value in pipeline.stream_stl(reference, mt_text, lxx_text):
                        if key is None:
            ...
            _t = threading.Thread(target=_run_stl, daemon=True)
            _t.start()
